...
def plot_result(pil_img, prob, boxes, save_name=None, imshow=False, imwrite=False):
    ...
    for p, (xmin, ymin, xmax, ymax) in zip(prob, boxes):
        ...
        crop_img = opencvImage[int(ymin):int(ymax), int(xmin):int(xmax)]
        cv2.imwrite('./result/crop/crop_{}'.format(save_name), crop_img)

        cv2.rectangle(opencvImage, (int(xmin), int(ymin)), (int(xmax), int(ymax)), (50, 50, 50), 2)
...
# 单张图像的推断
def detect(im, model, transform, prob_threshold=0.5):
...
if __name__ == "__main__":
    ...
    files = os.listdir("./myData/coco/val")

    for file in files:
        img_path = os.path.join("./myData/coco/val", file)
