...
def compute_mIoU(gt_dir, pred_dir):  # 计算mIoU的函数
    ...
    name_classes = ['background', 'lesion']
    hist = np.zeros((2, 2))
    ...
    for i, j in zip(glob.glob(pred_dir), glob.glob(gt_dir)):
    #for ind in range(len(gt_imgs)):  # 读取每一个（图片-标签）对
        pred = np.array(Image.open(i))  # 读取一张图像分割结果，转化成numpy数组
        label = np.array(Image.open(j))  # 读取一张对应的标签，转化成numpy数组
...
if __name__ == '__main__':
    path1 = 'C:/Users/tanyi/Swin-Seg-main/tools/blurred_mask_eval/*.png'
    path2 = 'C:/Users/tanyi/Swin-Seg-main/tools/data/ade/test/*.png'

    compute_mIoU(path2, path1)  # 执行主函数 两个路径分别为 ‘ground truth’,'自己的实验分割结果'
